...
def for_computation(current_idx: int):
    ...
    if sol_cvx['status'] in ['optimal', 'optimal_inaccurate']:
        ...
        states_rtn_cvx_i = roe_to_rtn(states_roe_cvx_i, prob.oe)  # Transpose back to (n_time, 6)
...
if __name__ == '__main__':
    ...
    save_fname = 'v08_w3_seen_test_test'
    tailored_command = True  # whether to use tailored command inputs (with waypoint info)
    unseen_command = False  # whether to use unseen commands (only works when tailored_command=True)
    master_file = 'commands_summary_w3_train.jsonl'  # doesn't matter, deprecated
