...
def for_computation(input_iterable):
    ...
    text_encoder = input_dict['text_encoder']
    command_mapping = input_dict['command_mapping']
    test_loader = input_dict['test_loader']
    scp = input_dict['scp']
    tailored_command = input_dict['tailored_command'] 
    ...
    base_seed = input_dict.get('base_seed', 12345)
    ss = np.random.SeedSequence([base_seed, int(current_idx)])
    ...
    # print("behavior:", behavior_i, " | command_id:", command_id_i, " | text_command:", text_command)
    ...
    if sol_cvx['status'] in ['optimal', 'optimal_inaccurate']:
        ...
        states_rtn_cvx_i = roe_to_rtn(states_roe_cvx_i, oe_ref)
        ...
        if scp:
            ...
            if feas_scp_i in ['optimal', 'optimal_inaccurate']:
                ...
                # save SCP trajectory
                roef_scp = sol_scp['z']['state'][-1] + cim_f @ sol_scp['z']['action'][-1]
                rtnf_scp = roe_to_rtn(sol_scp['z']['state'], oe_ref)  

                out['roe_scp_cvx'] = np.concatenate((sol_scp['z']['state'], roef_scp.reshape(1, -1)), axis=0)
                out['rtn_scp_cvx'] = np.concatenate((rtnf_scp, (psi_f @ roef_scp).reshape(1, -1)), axis=0)
...
if __name__ == '__main__':
    ...
    num_processes = 10
    N_data = 30  # number of data to try warm-start analysis 
    scp = True           # whether to solve SCP after CVX/ART

    save_fname = 'v08_w3_unseen_test'
    ...
    # Orin cannot handle multiprocessing (passing Tensor objects between processes)
    is_orin = False 
    ...
    # input aggregation 
    inputs = list(zip(np.arange(N_data_test), itertools.repeat(other_args)))
    if not is_orin: 
        # Pool creation --> Should automatically select the maximum number of processes
        p = Pool(processes=num_processes)
        iterator = p.imap(for_computation, inputs) 
    else: 
        # plain Python loop, no multiprocessing
        def seq_iter():
            for x in inputs:
                yield for_computation(x)
        iterator = seq_iter()
