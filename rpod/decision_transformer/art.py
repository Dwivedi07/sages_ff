...
class AutonomousRendezvousTransformer_inv(DecisionTransformerPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        self.predict_constraint = torch.nn.Linear(config.hidden_size, 1)
    ...
    def forward(
        ...
        constraint_embeddings = self.embed_constraint(constraints)
        time_embeddings = self.embed_timestep(timesteps)
        ...
        # this makes the sequence look like (s_f, a_f, r_f, c_f, s_1, a_1, r_1, c_1, ...)
        # which works nice in an autoregressive sense since states predict actions
        stacked_inputs = (
            torch.stack((state_embeddings, action_embeddings, reward_embeddings, constraint_embeddings), dim=1)
            .permute(0, 2, 1, 3)
        ...
        # reshape x so that the second dimension corresponds to the original
        # states (0), actions (1), reward (2), constraint (3); i.e. x[:,1,t] is the token for a_t
        ...
        reward_preds = self.predict_reward(x[:, 1])  # predict next reward given state and action
        constraint_preds = self.predict_constraint(x[:, 1])  # predict next constraint given state and action
        if not return_dict:
            return (state_preds, action_preds, reward_preds, constraint_preds)
...
class AutonomousRendezvousTransformerLang(DecisionTransformerPreTrainedModel):
    """
    ...
    def forward(
        ...
        action_embeddings = self.embed_action(actions)        # [B, T, H]
        command_embeddings = commands_emb                     # [B, M, H]   

        # allow single-vector command [B, H]
        if command_embeddings.dim() == 2:
            command_embeddings = command_embeddings.unsqueeze(1)

        time_embeddings = self.embed_timestep(timesteps)      # [B, T, H]

        # add time embeddings only to trajectory tokens
        constraint_embeddings = constraint_embeddings + time_embeddings
        state_embeddings  = state_embeddings + time_embeddings
        action_embeddings = action_embeddings + time_embeddings
        # command_embeddings: no time embedding
        ...
        # attention mask: ones for commands + repeated mask for (c,s,a)
        ...
        x = encoder_outputs[0]  # [B, M + 3*T, H]

        # drop command block, reshape remaining to [B, 3, T, H]
        M = command_embeddings.shape[1]
        x_traj = x[:, M:, :].reshape(batch_size, seq_length, 3, self.hidden_size).permute(0, 2, 1, 3)
