...
from freeflyer.decision_transformer._art_closed_loop import AutonomousFreeflyerTransformerMPC, ConvexMPC, MyopicConvexMPC
...
if __name__ == '__main__':
    ...
    for n_steps in mpc_steps_analysis:
        ...
        for i in tqdm(range(len(uniform_idx)), total=len(uniform_idx)):
            ...
            if i%200 == 0:
                np.savez_compressed(root_folder + '/optimization/saved_files/closed_loop/ws_analysis_time_DAGGER' + str(other_args['mpc_steps']) + '_' + str(i),
                            J_cvx = J_cvx,
                            J_cvxMPC = J_cvxMPC,
        ...
        np.savez_compressed(root_folder + '/optimization/saved_files/closed_loop/ws_analysis_time_DAGGER' + str(other_args['mpc_steps']),
                        J_cvx = J_cvx,
                        J_cvxMPC = J_cvxMPC,
                        J_myocvxMPC = J_myocvxMPC,
                        J_artMPC = J_artMPC,
