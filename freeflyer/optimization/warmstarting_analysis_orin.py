...
_worker_sample_init_final = None

def init_worker(model_name, model_state, text_state, 
                test_loader, command_mapping, scp, tailored_command, sample_init_final, max_tokens):
    global _worker_model, _worker_text_encoder, _worker_data_stats
    global _worker_test_loader, _worker_command_mapping
    global _worker_scp, _worker_tailored_command, _worker_sample_init_final
    ...
    _worker_sample_init_final = sample_init_final
...
def for_computation(current_idx: int):
    ...
    model_ctg           = _worker_model
    text_encoder_ctg    = _worker_text_encoder
    test_loader     = _worker_test_loader
    command_mapping = _worker_command_mapping
    data_stats = _worker_data_stats
    unseen_text= _worker_tailored_command
    sample_init_final = _worker_sample_init_final
...
if __name__ == '__main__':
    ws_version = 'v_06_orin_80k_unseen'  # warmstarting analysis version #'v_01_total_seen_fo'. #
    ...
    model_ctg.to("cpu").eval()
    model_ctg_state = model_ctg.state_dict()
    model_text = DT_manager.get_DT_model(model_version_text, train_loader, eval_loader, ctg_condition = False)
    model_text.to("cpu").eval()
    model_text_state = model_text.state_dict()

    # load the text encoders for each models (and its weight)  
    MODEL = os.getenv("FTA_MODEL", "distilbert-base-uncased")
    text_encoder_ctg = FrozenTextAdapter(model_name=MODEL, out_dim=model_ctg.hidden_size, output_mode="tokens").to("cpu").eval()
    ...
    text_state_ctg = text_encoder_ctg.state_dict()
    del text_encoder_ctg, model_ctg

    text_encoder_text = FrozenTextAdapter(model_name=MODEL, out_dim=model_text.hidden_size, output_mode="tokens").to("cpu").eval()
    ...
    text_state_text = text_encoder_text.state_dict()
    del text_encoder_text, model_text
    ...
    with Pool(processes=num_processes, initializer=init_worker, 
              initargs=(model_version_ctg, model_ctg_state, text_state_ctg,
              test_loader, command_mapping, sample_init_final)) as p:  
